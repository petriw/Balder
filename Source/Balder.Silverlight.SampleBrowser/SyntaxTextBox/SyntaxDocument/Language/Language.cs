...
namespace Fireball.Syntax
{
  ...
  public class Language
  {
    ...
    private Dictionary<BlockType, BlockType> _Blocks = new Dictionary<BlockType, BlockType>();
    private Dictionary<TextStyle, TextStyle> _Styles = new Dictionary<TextStyle, TextStyle>();
    ...
    public void SaveStyles()
    {
      ...
      XElement xwr = new XElement("styles");
      ...
      foreach (TextStyle style in styles)
      {
        XElement xstyle = new XElement("Style",
          ...
          new XAttribute("ForeColor", style.ForeColor.ToHexString()),
          new XAttribute("BackColor", style.BackColor.ToHexString()),
          new XAttribute("Bold", style.Bold.ToString()),
          new XAttribute("Italic", style.Italic.ToString()),
          new XAttribute("Underline", style.Italic.ToString()));
        xwr.Add(xstyle);
      }
      FileStream stream = System.IO.File.OpenWrite(path);
      byte[] data = Encoding.UTF8.GetBytes( xwr.ToString());
